...
plugin.load = function(arg, next) {
	...
	router.get( '/authenticate', cas.bounce, function ( req, response ) {
		...
		passport.use(new passportLocal({passReqToCallback: true}, plugin.getUidBySchoolnumber(schoolnumber, function(err, uid) {
			if (err) {
					console.log('login with schoolnumber checkField failed');
					return callback(err);
				}
			console.log('uid :' + uid);
			if (uid !== null) {
		...
		// var username = nconf.get('mongo:username');
		...
		// 	if (err) throw err;
		// 	var obj = db.collection('objects');
        //
		// 	obj.find({'schoolnumber' : schoolnumber}).toArray(function(err, res) {
		// 		if (err) {
		// 			console.log(err);
		...
		// 		}else {
		// 			console.log(res);
		// 			if (Array.isArray(res) && res[0] && res[0]['schoolnumber']) {
		// 				// 模拟用户登录
        //
		// 				passport.use(new passportLocal({
		// 					passReqToCallback: true
		// 				}, function(req_, username_, password_, next) {
		// 					var key = res[0]['_key'];
		// 					var data = key.split(':');
		// 					console.log(data);
        //
		...
		// 						}
		// 					], next);
		// 					response.redirect(nconf.get('relative_path') + '/');
		// 				}));
        //
		// 			}else {
		// 				// 该用户未注册，重定向至登录页面
...
plugin.getUidBySchoolnumber = function(schoolnumber, callback) {
	...
	if (username && password && host && port && database) {
		...
		MongoClient.connect(url, function(err, db) {
			...
			obj.find({'schoolnumber' : schoolnumber}).toArray(function(err, res) {
				if (err) {
					...
					if (!empty(res) && res[0]) {
						console.log('schoolnumber Existing')
						var key = res[0]['_key'];
						var data = key.split(':');
		...
		/*
		db.getObject('user:uid:ns:custom_fields', schoolnumber, function(err, uid) {
			if (err) {
				return callback(err);
			}
			console.log(uid);

...
plugin.load = function(arg, next) {
	...
	router.get( '/authenticate', cas.bounce, function ( req, res ) {
		...
		ifUserExist = function(num, callback) {
			db.getObjectFields('schoolnumber', num, function(err) {
						if (err) {
							callback(err);
						}else {
							callback();
		...
		ifUserExist(schoolnumber, function(err) {
			if (!err) {
				console.log('exist');
			}else {
		...
		res.send( '<html><body>Hello!'+ username +'</body></html>' );
	});

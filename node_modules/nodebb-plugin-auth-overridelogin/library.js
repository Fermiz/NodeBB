...
var customFields = {
	...
	//console.log('cas ' + nconf.get('cas:service_url'));
	var cas = new CASAuthentication({
		cas_url     : 'https://uis.nwpu.edu.cn/cas',
		service_url : nconf.get('cas:service_url'),
		cas_version : '2.0'
	});
	...
	router.get( '/authenticate', cas.bounce, function ( req, response ) {
		...
		plugin.getUidBySchoolnumber(schoolnumber, function(err, uid) {
			...
			if (uid !== null) {// 已经注册过， 直接登录
				// passport.use(new passportLocal({passReqToCallback: true}, plugin.signinWithSchoolnumber));
				response.redirect(nconf.get('relative_path') + '/login');
			}else {// 没有学号，开启注册
...
plugin.signinWithSchoolnumber = function(req, username, password, next) {
	var schoolnumber = req.session[cas.session_name];
	...
	for(var key in customFields) {
		...
		switch(key) {
			case 'schoolnumber':
				...
				var html = '<input class="form-control" type="text" name="schoolnumber" id="schoolnumber" value="' + schoolnumber + '" placeholder="输入学号/工号"><span class="custom-feedback" id="schoolnumber-notify"></span>';
				var label = "学号/工号";
				break;
		}
